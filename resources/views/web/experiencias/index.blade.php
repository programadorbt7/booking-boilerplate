...
@section('metaSEO')
    <title>Experiencias - {{ $nameEnterprise }}</title>
    ...
    <meta name="keywords"
        content="Viajes, Experiencias, Personas, Parques, Lugares, Entidades, Personas, Reserva, Tour, Aventura, Playas, Turismo, Visitas, Funtastic">
@endsection
...
@section('contenido-principal')
    ...
    {{-- MAIN --}}
    <section class="tour-type-two mt-5 mb-5 pb-5">
        <div class="container">
            <div class="row">
                {{-- FILTROS --}}
                <div class="col-lg-4">
                    <aside id="filtros-nav" class="tour-listing-sidebar">
                        <div class="tour-listing-sidebar__form tour-listing-sidebar__item wow animated fadeInUp"
                            ...
                            <div class="tour-listing-sidebar__amenities">
                                ...
                                <div class="tour-listing-sidebar__amenities-box" id="paisTour">
                                </div>
                            </div>
                            {{-- Estados --}}
                            <div class="tour-listing-sidebar__amenities">
                                ...
                                <div class="tour-listing-sidebar__amenities-box" id="estadoTour">
                                </div>
                            </div>
                            {{-- Tipo --}}
                            <div class="tour-listing-sidebar__amenities">
                            ...
                            <div class="tour-listing-sidebar__amenities">
                                ...
                                <div class="tour-listing-sidebar__amenities-box" id="categoriaTour">
                                </div>
                            </div>
                        </div>
                    </aside>
                            ...
                            @if ($count > 0)
                                @foreach ($experienciasList as $i => $experiencia)
                                    @php
                                        ...
                                        $categoriaExperiencias = $experiencia != null ? $fn->reemplaza_espacios($experiencia['categorias']) : '';
                                        $categoriaExperiencias = explode(',', $categoriaExperiencias);
                                    @endphp
                                    <div class="col-lg-12 col-md-12 wow fadeInUp filaTour inforTourPad estado_{{ $fn->eliminar_acentos($estadoExperiencias) }} tipo_{{ $fn->eliminar_acentos($tipoExperiencias) }} pais_{{ $fn->eliminar_acentos($paisExperiencias) }} 
                                        @if (is_array($categoriaExperiencias)) @for ($i = 0; $i < count($categoriaExperiencias); $i++)
                                                categoria_{{ $fn->eliminar_acentos($categoriaExperiencias[$i]) }}
                                            @endfor
                                        @else
...
@section('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            @php
                $filtrosPaisesTours = array_count_values($paisesExperiencias);
                $filtrosEstadosTours = array_count_values($estadosExperiencias);
                $filtrosTipoTours = array_count_values($tiposExperiencias);
                $filtrosCategoriaTours = array_count_values($categoriasExperiencias3);

            @endphp
            @php $n = 0; @endphp
            @foreach ($filtrosPaisesTours as $filtroTourName => $filtroTourCount)
                $("#paisTour").append(
                    '<li style="margin: 0.5rem; list-style: none;"><input class="form-check-input" type="checkbox" value="pais_{{ $fn->stringToUrl($filtroTourName) }}"> {{ $filtroTourName }} <span class="iconCountdates" style="float: right;">({{ $conteoPaisExperiencia[$n] }})</span></li>'
                );
                @php $n++; @endphp
            @endforeach

            @php $n = 0; @endphp
            @foreach ($filtrosEstadosTours as $filtroTourName => $filtroTourCount)
                $("#estadoTour").append(
                    '<li style="margin: 0.5rem; list-style: none;"><input class="form-check-input" type="checkbox" value="estado_{{ $fn->stringToUrl($filtroTourName) }}"> {{ $filtroTourName }} <span class="iconCountdates" style="float: right;">({{ $conteoEstadoExperiencia[$n] }})</span></li>'
                );
                @php $n++; @endphp
            @endforeach
            ...
            @php $n = 0; @endphp
            @foreach ($filtrosCategoriaTours as $filtroTourName => $filtroTourCount)
                $("#categoriaTour").append(
                    '<li style="margin: 0.5rem; list-style: none;"><input class="form-check-input" type="checkbox" value="categoria_{{ $fn->stringToUrl($filtroTourName) }}"> {{ $filtroTourName }} <span class="iconCountdates" style="float: right;">({{ $conteoCategoriaExperiencia[$n] }})</span></li>'
                );
                @php $n++; @endphp
            @endforeach
