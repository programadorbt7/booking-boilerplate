...
@section('contenido-principal')
    ...
    <section class="checkout-page section-space">
        <div class="container">
            <form class="checkout-page__form form-one" id="frmCompra" action="{{ route('save-openpay-hotel') }}"
                method="post">
                @csrf
                ...
                <input type="hidden" name="nombrehotel" id="nombrehotel" value="{{ $hotelName }}">
                <input type="hidden" name="gtotal" id="gtotal" value="{{ $pr }}">

                <input type="hidden" name="marte" value="{{ $markup }}">
                <input type="hidden" name="hash" value="{{ $hash }}">
                <input type="hidden" name="idratehawk" value="{{ $id }}">
                ...
                <input type="hidden" name="hotelbd" value="{{ $idhotelbd }}">
                                        ...
                                        @foreach ($sitioweb[0]->terminales as $terminal)
                                            @if ($terminal->id == 3)
                                                ...
                                                <div class="col-md-4 text-center">
                                                    {{-- <input type="submit" value="Confirmar reservación" class="btn btnDatosCompra"
                                                id="btnPagar" style="border-radius:20px;"> --}}
                                                    ...
                                                    <p class="textCardPayment">Realiza tu pago de forma segura con Openpay
                                                        a través de BBVA. Solo se aceptan tarjetas mexicanas.</p>
                                                </div>
                                            @endif
                    ...
                    <div class="col-xl-4 col-lg-4">
                        <div class="checkout-page__your-order">
                            ...
                            <table class="table table_summary">
                                <tbody>
                                    <tr>
                                        <td class="bold">Nombre de hotel</td>
                                        <td>{{ $hotelName }}</td>
                                    </tr>
                                    <tr>
                                        <td class="bold">Tipo de Habitación</td>
                                        <td>{{ $room }}</td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td class="bold">Adultos</td>
                                        <td>{{ $adults }}</td>
                                    </tr>
                                    <tr>
                                        <td class="bold">Menores</td>
                                        <td>{{ $menoresHabitacion }}</td>
                                    </tr>
                                </tbody>
                                    ...
                                    <tr class="total_row">
                                        ...
                                        <td colspan="2" class="text-right">
                                            <input name="total" class="w-100 text-right border-0 bg-transparent bold"
                                                id="total"
                                                value="{{ "$ " . $fn->moneda($pr) . ' ' . $monedaSeleccionada }}"
                                                disabled>
                                            <input type="hidden" name="totalConCupon" id="totalHoteleriaFinal"
                                                value="{{ $fn->moneda($pr) }}">
                                        </td>
                                    </tr>
                                </tfoot>
...
@section('scripts')
    ...
    @if ($paypal == 1)
        <script
            src="https://www.paypal.com/sdk/js?client-id={{ $paypalClientId }}&currency={{ $cur }}&disable-funding=credit,card">
        </script>
        <script>
            paypal.Buttons({
                createOrder: function(data, actions) {
                    var totalPaypal = $('#gtotal').val();
                ...
                $.ajax({
                    ...
                    .done(function(e) {
                        if (e.status === 'COMPLETED') {
                            ...
                            let url = "gracias-openpay-hotel?id=" + transaccion + "&terminal=4&monto=" + total +
                                "&order=" + orderID;
                            location.href = url;
                        } else {
    ...
    <script>
        ...
        const mensajeFrontEnd = (mensajeCupon, validStatus, total, descuento = 0, hasDescuent) => {
            ...
            let bonos = descuento;
            let gTotalJs = total;
            let phpTotal = "@php echo $fn->moneda($pr) . ' ' . $monedaSeleccionada @endphp";
